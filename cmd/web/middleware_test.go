...
func TestSecureHeader(t *testing.T) {
	// Initialize a new httptest.ResponseRecorder and dumy http.Request.
	...
	// Pass the mock HTTP handler to our secureHeaders middleware. Becuase 
	// secureHeaders *returns* a http.Hanlder we can call its ServeHTTP()
	// method, passing in the http.ResponseRecorder anddummy http.Request to execute it.
	secureHeaders(next).ServeHTTP(rr, r)

	// Call the Result() method on the http.ResponseRecorder to get the results of the test.
	rs := rr.Result()

	// Check that the middleware has correctly set the Content-Secure-Policy header on the response.
	...
	// Check that the middleware has correctly called the next handler in line and the 
	// response status code and body are as expected.
